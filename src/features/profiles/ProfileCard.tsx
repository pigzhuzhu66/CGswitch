      ...
      <div className="flex min-w-0 flex-1 items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="profile-card-meta muted mt-1 flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-1 text-xs">
            ...
            {profile.admin_url ? <button type="button" className="grid h-4 w-4 place-items-center rounded-full text-accent transition-colors hover:bg-(--profile-chip-bg)" title="打开官网" aria-label="打开官网" onClick={(event) => { event.stopPropagation(); void api.openUrl(profile.admin_url!).catch((error) => feedback.error(String(error))); }}><ExternalLink className="h-3.5 w-3.5" strokeWidth={2} aria-hidden="true" /></button> : null}
          </div>
        </div>
      </div>
